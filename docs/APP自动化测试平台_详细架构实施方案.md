...
1. **多用户绑定与权限**：
   ...
   - `POST /api/agent/bind`：绑定阶段只校验 `user_key+install_id`，不接收、不校验本机旧机器 PSK；有效用户 Key 可创建、恢复或重复绑定安装实例。每次绑定成功均旋转并返回新的机器 PSK，供后续 WS、设备上报、上传和机器侧解绑认证；同一用户重复绑定时同时刷新其撤销凭据。用户 Key 不以明文写入服务端数据库或日志；桌面 Agent 在绑定成功后可将最近使用的 Key 明文保存为 EXE 运行目录下的 `user_key.txt`，启动时仅以掩码回填。
...
2. Action 节点执行后继续按顺序执行 Assertion 节点。断言 `max_wait_seconds` 范围为 0–300，默认 10 秒，0 表示只检查一次；条件未满足、元素暂时不存在或 stale 才按 500ms 间隔重试，不可恢复的配置/驱动错误立即失败。
...
12. **敏感执行协议**：执行快照固化敏感变量名称及节点/步骤/断言参数路径；Worker 下发真实值和脱敏元数据，Agent 执行动作仍使用真实值，但本地日志、WS 回传和 Backend 落库/广播、详情与报告均按元数据脱敏。涉及敏感变量的 Agent 协议和最低版本为 `3.4.0`，低于该版本的 Agent 必须拒绝接入或执行。
