...
元素修改后，引用该元素的用例会使用最新公共元素配置。不同 APP 的定位差异应直接维护公共智能定位或拆分公共元素，APP 档案不会替换元素定位。
...
用例变量已经规范化为带稳定 `variable_id` 的正式变量。新建用例时先保存用例，再在变量管理入口中按 `scope=case` 维护该用例的变量；请求不会再把变量写入用例 JSON。步骤和断言中的 `${变量名}` 只表示引用。

项目、套件和用例变量的公共值由有权限的项目成员维护。密码、Token 等变量可标记为敏感，敏感值保存后只显示掩码，不会在执行详情或报告中回显；修改敏感变量的其它属性时，不要把掩码当作新值提交。
...
APP 档案 occurrence 能力变量属于设备能力值，优先级最高；当前用户的“我的变量配置”只替换指定 `variable_id` 的公共项目、套件或用例定义，不能突破该能力值。同名变量必须依据所属项目、套件或用例身份区分。
...
- 单个用例执行：适合调试。
- 单个套件执行：适合一次回归一组相关功能。
- 运行已选套件：适合某个 APP 版本的整体回归，也可先取消本次不参加的套件。
- 批量执行：适合一次选择多个套件或用例。
...
APP 档案是设备能力档案：新建后默认支持全部公共测试资产，只有明确配置为不支持的套件、编排项、步骤或断言才会跳过。它不复制用例、元素或变量，也不替换公共元素定位。

### 12.1 创建与发布

1. 进入项目工作区的“APP 档案”，点击“新建档案”。
2. 填写名称和描述并保存。
3. 在“发布版本”中创建与 APP 构建对应的版本。
4. 执行前选择档案、发布版本和在线空闲设备，先查看执行预览。

档案修订号由 Owner/Admin 管理的跳过规则和档案能力变量变化推进；当前用户的“我的变量配置”不推进共享档案修订号。

### 12.2 工作区与 occurrence 身份

工作区按套件、套件前后置、用例、步骤和断言展示状态。套件中的每一行用例都是一个精确的编排项 occurrence，身份是 `suite_case_id`；同一用例在不同套件或同一套件重复编排时，必须分别配置，不能用 `case_id` 猜测目标。

页面常用操作包括刷新、运行已选套件、发布版本、差异清单和“我的变量配置”。差异清单只展示跳过规则和档案能力变量。

### 12.3 设置和恢复跳过规则

对当前 APP/设备不支持的内容点击“跳过”，选择“不支持、未适配、已废弃、环境限制或其他”并填写必要说明。套件使用 `suite_id`；套件内用例、步骤和断言使用对应 occurrence 的 `suite_case_id`（步骤/断言还带节点标识）；套件前后置步骤使用 `suite_id` 和节点标识。

恢复后，下级会重新按自己的直接规则计算。整套跳过的套件不进入执行；套件内被跳过的内容在详情和报告中显示 N/A，不计入成功率分母。

### 12.4 配置档案能力变量

用例行中的“档案能力变量”用于配置当前 occurrence 的设备能力值，例如固定端口或设备专属能力参数。它由 Owner/Admin 管理，对所有用户生效，作用于该编排项引用该变量的全部步骤和断言，并且优先级最高，连本次执行参数也不能突破。恢复后回到继承值。

### 12.5 我的变量配置

点击档案页顶部“我的变量配置”，只管理当前登录用户在当前档案中的私有覆盖：

1. 按项目、套件或用例筛选变量；同名 `username` 按稳定 `variable_id` 及所属套件/用例分别显示。
2. 查看“公共值、我的值、当前展示值、是否已覆盖和引用次数”，展开行可查看真实动作/断言参数中的引用位置；元素智能定位不算变量引用。
3. 非敏感变量可编辑并保存；空字符串是合法值。点击恢复发送 `null`，删除个人覆盖并恢复公共值。
4. 敏感变量只显示掩码或“已配置”，编辑时需重新输入；敏感值不会回显到页面、日志、执行详情或报告。

Owner/Admin/Member 可按项目权限查看并修改自己的值；Viewer 不可查看也不可修改“我的变量配置”入口，相关 GET/PATCH 请求均返回 403。保存请求使用 `request_id` 幂等；没有用户变量 revision，合法的新请求按最后写入生效。用户变量不会改变共享档案 revision。

最终优先级为：

```text
APP 档案 occurrence 能力变量
  > 本次执行参数
  > 编排项变量
  > 套件变量（用户私有值替换公共定义）
  > 用例变量（用户私有值替换公共定义）
...
步骤变量不参与编辑，只作为引用或运行时输出。单用例从套件选择执行时，应传递准确的 `context_suite_case_id`；重复编排不能按名称或 `case_id` 选择。

### 12.6 执行与安全提示

执行预览会显示实际可执行内容和 N/A 排除项。敏感执行要求 Agent 协议和版本至少为 `3.4.0`；旧 Agent 必须升级，不会静默接收敏感执行负载。执行快照、执行详情、报告和 HTML 报告均不会展示敏感变量明文；重试沿用原执行快照。
...
如果另一个 APP 版本有不支持的测试内容或固定设备能力值：

1. 在“APP 档案”创建“移动 APP 新版本”。
2. 创建对应发布版本。
3. 对不支持的套件/编排项设置跳过；对需要固定的当前编排项配置“档案能力变量”。
4. 在“差异清单”确认只有预期的跳过和能力变量变化。
...
- 公共元素优先集中维护；APP 档案只配置跳过规则和 occurrence 能力变量。
