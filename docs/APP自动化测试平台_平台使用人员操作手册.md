...
- “刷新”：重新读取最新配置。
- “运行已选套件”：执行当前档案中本次勾选的套件；可执行套件默认选中，页面筛选、
  展开、翻页和刷新不会改变选择。顶部会显示已选计数，并可用“恢复全部选择”清除
  用户取消项。
- “发布版本”：创建和管理档案版本。
- “差异清单”：查看档案相对于公共配置的差异。
- “覆盖配置”：配置元素、变量或节点覆盖。
- “搜索套件”：快速定位套件。
- “全部状态”：按正常、已跳过、已覆盖筛选。直接跳过的套件会自动取消且禁用选择，
  恢复后默认重新选中。
...
1. 在档案工作区对对应行点击“跳过”；套件执行复选框只用于本次运行，不会修改档案规则。
2. 在“设置跳过规则”窗口选择“原因”：
...
恢复时点击对应行的“恢复”。整套直接跳过的套件不会进入本次执行报告；套件内用例、
步骤或断言的跳过内容仍显示为 N/A，不会被误判为失败。
...
5. 按需要取消不参加本次执行的套件，点击“运行已选套件”；也可以对单个套件/用例点击“运行”。
6. 在设备选择窗口选择在线空闲设备。
7. 确认后开始执行。
