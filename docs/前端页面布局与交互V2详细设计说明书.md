...
| `/projects/:projectId/executions` | 项目执行 | viewer+ | 固定筛选当前项目 |
| `/projects/:projectId/executions/:executionId` | 项目执行详情 | viewer+ | 保持项目工作区 |
| `/projects/:projectId/reports` | 项目报告 | viewer+ | 固定筛选当前项目 |
| `/projects/:projectId/reports/:reportId` | 项目报告详情 | viewer+ | 保持项目工作区 |
...
全局导航固定为：工作台、项目、执行中心、设备中心、报告。进入项目后，侧栏整体切换为项目工作区，不再与全局菜单混排；项目工作区包含返回全部项目、项目身份、概览、用例、套件、元素、变量、执行、报告、设置和返回全局工作台。
...
- 详情上下文跟随入口：项目入口使用 `/projects/:projectId/...`，全局入口继续使用 `/executions/...` 或 `/reports/...`；不按最近项目猜测上下文。
- 项目内运行、重试、执行转报告和报告转执行继承当前项目工作区；详情 URL 中项目 ID 与资源实际归属不一致时规范化为真实项目 ID。
...
├────────────────────┬───────────────────────────────────────────┤
│ 大纲               │ 步骤编辑区                                │
│ 前置操作            │ 卡片头：阶段内序号、动作、拖拽、删除       │
│ 主体步骤            │ 卡片体：元素、参数、描述、失败后继续       │
│ 后置操作            │ 三阶段分别维护，主体失败仍尝试后置         │
│ 断言                │                                           │
│ 变量                │                                           │
├────────────────────┴───────────────────────────────────────────┤
│ 校验摘要                                  [取消] [保存草稿/保存] │
...
- 步骤 `phase` 为 setup/main/teardown，历史缺省值归一为 main；每个阶段的 `order` 分别从 1 连续递增，动作必须来自 Action Registry。
...
- 单用例和套件的 DevicePicker 提供“执行前置操作/执行后置操作”开关，默认关闭；套件逐个用例应用。单用例额外提供“复用设备当前界面，不启动 APP”，选中后明确提示会跳过 `launch_app`。
...
**路由**：全局 `/executions`；项目内 `/projects/:projectId/executions`。登录用户。
...
- 项目路由固定传 `project_id`，隐藏冗余项目列；全局路由保持跨项目列表。

**接口**：执行列表、停止、重试、默认设备和报告查找接口。

### 5.14 执行详情

**路由**：全局 `/executions/:executionId`；项目内 `/projects/:projectId/executions/:executionId`。项目 viewer+。

```text
┌ 返回执行中心  执行 #1001 / 登录套件  [运行中]   [停止] [重试] ┐
│ 项目 / 设备 / 创建人 / 开始时间 / 耗时 / 超时 / 执行参数      │
├─────────────────────────────────────┬──────────────────────────┤
│ 用例、步骤、断言时间线              │ 实时日志                 │
...
- 前置和后置步骤显示阶段标签；报告步骤表同步显示主体/前置/后置阶段。
- 执行摘要展示顶层 `parameters`；每个步骤展示变量渲染后的 `parameters`。Agent 每个步骤结束后上报日志，页面通过 WS 实时追加，刷新后由 REST 日志恢复。
...
- 返回按钮优先返回真实来源；直接链接按当前工作区回退到项目或全局执行列表。

**接口**：执行详情、日志、停止、重试、执行附件、报告查找和执行 WebSocket。

### 5.15 报告中心

**路由**：全局 `/reports`；项目内 `/projects/:projectId/reports`。登录用户。
...
- 项目路由固定传 `project_id` 并隐藏项目列，详情链接保持项目工作区。

**接口**：报告列表和下载接口。

### 5.16 报告详情

**路由**：全局 `/reports/:reportId`；项目内 `/projects/:projectId/reports/:reportId`。项目 viewer+。

**布局**：顶部执行摘要和顶层执行参数；第二行统计卡和状态分布；主体为用例结果（含步骤参数）；底部为完整日志。
...
└─ cases[]
   ...
   ├─ steps[]: order、action、parameters、status、duration、actual_value、error_message、artifact_id
...
```json
{
  ...
  "case_id":500,
  "step_order":2,
  "assertions":[{"type":"text_equals","status":"pass","expected":"登录成功","actual":"登录成功"}],
  "case_status":"passed",
  "timestamp":"..."
}
```
...
所有状态值使用小写。`step_result.status` 是步骤状态，`case_status` 是用例状态，前端不得混用；`completed` 到达时剩余用例必须同步收敛。消息通过 `execution_id + case_id + step_order + type` 幂等合并。执行终态关闭 WebSocket 后，日志连接标识显示“执行已结束”，不得回退为“连接中”。
