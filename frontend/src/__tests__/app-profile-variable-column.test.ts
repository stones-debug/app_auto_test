...
describe('APP 档案工作台「变量」列', () => {
  it('独立成列，位于「原因」列之前', () => {
    expect(source).toContain('label="档案能力变量"')
    expect(source.indexOf('label="档案能力变量"')).toBeLessThan(source.indexOf('label="原因"'))
  })

  it('变量不再挂在用例名称旁边', () => {
    expect(column('名称')).not.toContain('caseVariables(')
    expect(column('档案能力变量')).toContain('caseVariables(')
  })

  it('竖排展示「变量名：变量值」', () => {
    const block = column('档案能力变量')
...
describe('变量覆盖写入的版本号契约', () => {
  ...
  it('occurrence 保存每次真实变更生成新的幂等 request_id', () => {
    expect(source).toContain('request_id: createUuid()')
  })
