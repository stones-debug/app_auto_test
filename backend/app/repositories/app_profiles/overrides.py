...
async def list_suite_case_variable_overrides(
    db: AsyncSession, profile_id: int, suite_case_id: int
) -> list[AppProfileSuiteCaseVariableOverride]:
    rows = await db.execute(
...
async def list_suite_case_variable_overrides_batch(
    db: AsyncSession, profile_id: int, suite_case_ids: set[int]
) -> dict[int, list[AppProfileSuiteCaseVariableOverride]]:
...
async def get_suite_case_variable(
    db: AsyncSession, profile_id: int, suite_case_id: int, name: str
) -> AppProfileSuiteCaseVariableOverride | None:
    return (
        await db.execute(
            select(AppProfileSuiteCaseVariableOverride).where(
                AppProfileSuiteCaseVariableOverride.profile_id == profile_id,
...
async def upsert_suite_case_variable(
    db: AsyncSession, *, profile_id: int, suite_case_id: int, name: str,
    value: str, user_id: int,
) -> AppProfileSuiteCaseVariableOverride:
    row = await get_suite_case_variable(db, profile_id, suite_case_id, name)
    if row is None:
        row = AppProfileSuiteCaseVariableOverride(
            profile_id=profile_id, suite_case_id=suite_case_id, name=name,
            value=value, created_by=user_id, updated_by=user_id,
        )
        db.add(row)
        await db.flush()
    else:
