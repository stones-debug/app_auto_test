...
class SetSliderValueParams(ParamsBase):
    min_value: float
    max_value: float
    target_value: float
    value_element_id: int | None = None
    value_attribute: Literal['auto', 'text', 'content-desc', 'value', 'progress'] = 'auto'
    left_inset_percent: float = Field(default=3.0, ge=0, le=95)
    right_inset_percent: float = Field(default=3.0, ge=0, le=95)
    track_y_percent: float = Field(default=50.0, ge=0, le=100)
    ...
    max_adjustments: int = Field(default=3, ge=0, le=10)
    wait_timeout: int = Field(default=10, ge=0, le=300)

    @model_validator(mode="after")
    def _validate_numeric_range(self):
        if not math.isfinite(self.min_value) or not math.isfinite(self.max_value) or not math.isfinite(self.target_value):
            raise ValueError("min_value、max_value、target_value 必须是有限数字")
        if self.min_value >= self.max_value:
            raise ValueError("min_value 必须小于 max_value")
        if not self.min_value <= self.target_value <= self.max_value:
...
STEP_PARAM_MODELS: dict[str, type[ParamsBase]] = {
    ...
    'set_slider_value': SetSliderValueParams,  # noqa: F821
...
ELEMENT_PARAM_FIELDS: dict[str, tuple[str, ...]] = {
    'set_slider_value': ('value_element_id',),
}
